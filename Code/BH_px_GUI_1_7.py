...
class LIDARDataPx(Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.colorScaleMin=tk.StringVar()
        self.colorScaleMax=tk.StringVar()
        self.colorScaleMin.set('0')
        self.colorScaleMax.set('0')
        self.plotPixy=tk.StringVar()
        self.plotPixy.set('0')
        self.plotPixx=tk.StringVar()
        self.plotPixx.set('0')
...
class MainPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        plot_butt.grid(row=11, column=1, columnspan=2)

        fit_butt=ttk.Checkbutton(self, text='Fit instead of X-corr?', variable=controller.fit_var)
        fit_butt.grid(row=7, column=1)

        plot_pix_butt=ttk.Button(self, text='Plot individual pixel (x/y)', command=lambda:self.plot_individual_pixel(controller))
        plot_pix_butt.grid(row=8, column=1)

        plot_x = ttk.Entry(self, textvariable=controller.plotPixx)
        plot_x.grid(row=9, column=1)
        plot_y = ttk.Entry(self, textvariable=controller.plotPixy)
        plot_y.grid(row=10, column=1)
        ...
        ttk.Label(self, text="Color scale min and max: (max=0 for default)").grid(row=8, column=2)
        colorscale_min = ttk.Entry(self, textvariable=controller.colorScaleMin)
        colorscale_min.grid(row=9, column=2)
        colorscale_max = ttk.Entry(self, textvariable=controller.colorScaleMax)
        colorscale_max.grid(row=10, column=2)

    def plot_individual_pixel(self, controller):
        sdt_file=SdtFile(controller.filename)
        plt.plot(sdt_file.data[0][int(controller.plotPixy.get())][int(controller.plotPixx.get())])
        plt.show()
        ...
        if controller.filename=='':
            ...
            if MsgBox == 'yes':
                ...
                if controller.IRF_var.get() == 1:
                    ...
                    if int(controller.end_gate.get()) != 0:
                        ...
                        start_irf = 0
                        stop_irf = len(IRF_file_sdt.data[0][0])
                    if controller.bin_toggle.get() == 1:
                    ...
                    #scale it
                    m=np.amax(IRF_pix)/max_count_all
                    IRF_pix=IRF_pix/m
                    plt.plot(IRF_pix)
                    plt.show()    
               ...
               # fig2.suptitle('2D')
                ...
                print('Average max count: {}'.format(np.mean(max_arr)))
                #fig3.suptitle('Counts Map')    

                if controller.colorScaleMax.get()!='0':
                    fig7=go.Figure(data=[go.Heatmap(z=img_arr, colorscale='Jet', zmin=int(controller.colorScaleMin.get()), zmax=int(controller.colorScaleMax.get()),
                ...
                fig5=go.Figure(data=[go.Heatmap(z=img_arr, colorscale='Jet', zauto=True, colorbar=dict(thickness=80,
                               ticklen=3, tickcolor='black',
                               tickfont=dict(size=36, color='black')))])
                ...
                if controller.colorScaleMax.get()!='0':
                    fig8=go.Figure(data=[go.Surface(z=img_arr, colorscale='Jet', cmin=int(controller.colorScaleMin.get()), cmax=int(controller.colorScaleMax.get()),
                                   colorbar=dict(thickness=40,
                                   ticklen=3, tickcolor='black',
                                   tickfont=dict(size=9, color='black')))])
                    fig8.update_layout(font=dict(family="Arial",size=9,color="black"), scene=dict(aspectmode='manual',
                               aspectratio=aspect_dict))
                    fig8.show()
                
                fig6=go.Figure(data=[go.Surface(z=img_arr, colorscale='Jet', cauto=True, colorbar=dict(thickness=40,
                                   ticklen=3, tickcolor='black',
                                   tickfont=dict(size=9, color='black')))])
